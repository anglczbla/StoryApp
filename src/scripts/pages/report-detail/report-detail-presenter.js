export default class ReportDetailPresenter {
  ...
  async getCommentsList() {
    ...
    try {
      const comments = await this.#apiModel.getAllCommentsByReportId(this.#reportId);
      this.#view.populateReportDetailComments("Komentar ditemukan", comments);
    } catch (error) {
      console.error("getCommentsList: error:", error);
      this.#view.populateCommentsListError("Gagal memuat komentar");
    } finally {
  ...
  async postNewComment({ body }) {
    ...
    try {
      const response = await this.#apiModel.storeNewCommentByReportId(
        this.#reportId,
        { body }
      );
